# Compares three implementations of RWL1:
...
# --- BPDN ---
W_bpdn = zeros(n,nrw+1);
W_bpdn[:,1] = ones(n);
Xhat_bpdn = zeros(n,nrw);
for i ∈ 1:nrw
    xhat = Variable(n);
    problem = minimize( 1/2*sumsquares(y-Φ*xhat) + λ0*sum(W_bpdn[:,i].*abs(xhat)) );
    solve!(problem,SCSSolver(verbose=false),verbose=false);
    Xhat_bpdn[:,i] = evaluate(xhat);
    W_bpdn[:,i+1] = 1 ./ (abs.(Xhat_bpdn[:,i]) .+ ε);
end
errs_bpdn = [rerr(Xhat_bpdn[:,i]) for i ∈ 1:nrw];
...
# --- dynamic LCA ---
include("lca.jl");
Xhat_dlca, W_dlca = dlca(b, G, Tw, τ, τ, (nτ-1)*nrw*τ, zeros(n), λ0*ones(n), ε);
...
plot!(p, log.(errs_ilca), label = "LCA (iterative reweighting)", lw = 2,
    line = :dash);
for i in 1:nrw
    plot!(p, [(i-1)*nτ; i*nτ], log(errs_bpdn[i])*ones(2),
